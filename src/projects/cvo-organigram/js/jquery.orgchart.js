  ...
  OrgChart.prototype = {
      ...
      var $chart = this.$chart = $('<div>', {
        ...
        'click': function (event) {
          if (!$(event.target).closest('.node').length) {
            ...
            $chart.find('.nodeMenu').addClass('d-none');
          }
        }
      });
    ...
    attachExportButton: function () {
      ...
      var $exportBtn = $('<button>', {
        'class': 'oc-export-btn btn btn-dark btn-sm',
    ...
    setChartScale: function ($chart, newScale) {
      this.options.currentZoom = newScale;
    ...
    dropHandler: function (event) {
      ...
      const dropId = event.delegateTarget.id;
      const dropColor = this.getNode(dropId).className;

      // Dragged node data
      var $dragged = this.$chart.data('dragged');
      // console.log('$dragged:', $dragged)
      const draggedId = $dragged[0].id;
      const draggedNode = this.getNode(draggedId);
      const draggedColor = draggedNode.className

      // Change color of dragged node
      this.updateNode(draggedId, { className: dropColor });  // edit chart data
      $dragged[0].className = $dragged[0].className.replace(draggedColor, dropColor);  // edit DOM

      // Move dragged node in chart data
      this.deleteNode(draggedId);
      this.addNode(dropId, draggedNode);
    ...
    exportPDF: function (canvas, exportFilename) {
      ...
      doc.addImage(canvas.toDataURL(), 'png', 0, 0, docWidth, docHeight, "NONE");
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      html2canvas(sourceChart, {
        ...
        onclone: function (cloneDoc) {
          $(cloneDoc).find("#rootNode").html("<h5 style='margin-top:25px;color:#462282; font-weight: bold'>CVO GENT</h5>");
          $(cloneDoc).find(".show-info-icon").remove();
          $(cloneDoc).find(".show-node-menu-icon").remove();
          $(cloneDoc).find(".nodeMenu").remove();
    ...
    //
    // *****************************
    // CUSTOM FUNCTION: EXPORT JSON  
    // *****************************
    exportJSON: function (data, fileName = "export") {
      let blob = new Blob([JSON.stringify(data)], { type: 'application/json' })
      saveAs(blob, fileName + '.json')
    },
    // 
    // ********************************
    // CUSTOM FUNCTIONS: ZOOM ON CLICK
    // ********************************
    zoomInOnClick: function (e) {
      e.preventDefault();
      const newScale = this.options.currentZoom + 0.1;
      if (newScale > this.options.zoomoutLimit && newScale < this.options.zoominLimit) {
        this.setChartScale(this.$chart, newScale);
      }
    },
    //
    zoomOutOnClick: function (e) {
      e.preventDefault();
      const newScale = this.options.currentZoom - 0.1;
      if (newScale > this.options.zoomoutLimit && newScale < this.options.zoominLimit) {
        this.setChartScale(this.$chart, newScale);
    ...
    // ******************************
    searchNodes: function (value, arrNodes = null) {
      if (!arrNodes) {
        arrNodes = [this.options.data];
      }
      const arrResults = [];
      for (let node of arrNodes) {
        if (node.naam.toLowerCase().includes(value) || node.functie.toLowerCase().includes(value) || node.taken.toLowerCase().includes(value)) {
          arrResults.push(node);
        }
        if (node.children) { searchChildNodes(value, node.children); }
      }
      function searchChildNodes(value, childNodes) {
    ...
    // 
    showSearchResults: function (foundIds, noncollapsable = false) {
      // disable the expand/collapse feature
      if (noncollapsable) this.$chart.addClass('noncollapsable');
      // distinguish the matched nodes and the unmatched nodes according to the given key word
      this.$chart.find('.node').filter(function (index, node) {
        return foundIds.includes(node.id);
      }).addClass('matched').closest('.hierarchy').parents('.hierarchy').children('.node').addClass('retained');
      // hide the unmatched nodes
      this.$chart.find('.matched,.retained').each(function (index, node) {
      ...
      this.$chart.find('.matched').each(function (index, node) {
      ...
      loopChart(this.$chart.find('.hierarchy:first'));
    ...
    clearFilterResult: function () {
      $('.orgchart').removeClass('noncollapsable')
        .find('#rootNode').removeClass('slide-down');
      $('.orgchart')
    ...
    // 
    // **********************************
    // CUSTOM FUNCTIONS FOR CHART TOOLBAR
    // **********************************
    expandAll: function () {
      this.$chart.removeClass('noncollapsable');
      this.$chart.find('.matched').removeClass('matched');
      this.$chart.find('.retained').removeClass('retained');
      this.$chart.find('.hidden').removeClass('hidden');
      this.$chart.find('.slide-up').removeClass('slide-up');
      this.$chart.find('.slide-down').removeClass('slide-down');
      this.$chart.find('.slide-left').removeClass('slide-left');
      this.$chart.find('.slide-right').removeClass('slide-right');
      this.$chart.find('.hierarchy').attr('class', 'hierarchy');
    },
    //
    fit: function () {
      this.$chart.css("transform", '');
      this.options.currentZoom = 1;
    },
    // 
    refresh: function () {
      ...
      this.expandAll();
      this.$chart.removeClass('noncollapsable');
      const firstLevelNodes = this.options.data.children;
      firstLevelNodes.forEach(node => {
        this.hideChildren(this.$chart.find("#" + node.id));
      });
      this.fit();
    },
    //
    // ***************************************************
    // CUSTOM FUNCTIONS: CRUD FOR CHART DATA (not for DOM)  
    // ***************************************************
    // Get node from chart data (not from DOM hierarchy)
    getNode: function (targetId, arrNodes = null) {
      if (!arrNodes) {
        // arrNodes = [this.getHierarchy(true)];
        // arrNodes = this.options.data.children;
        arrNodes = [this.options.data];
      ...
      for (let node of arrNodes) {
        if (node.id == targetId) {
          ...
          const result = this.getNode(targetId, node.children);
          if (result) return result;
        }
      }
    },
    // Add new node to chart data
    addNode: function (targetId, newNode, arrNodes = null) {
      if (!arrNodes) {
        // arrNodes = this.options.data.children;
        arrNodes = [this.options.data];
      }
      for (let node of arrNodes) {
        if (node.id == targetId) {
          ...
          const result = this.addNode(targetId, newNode, node.children);
          if (result) return result;
        }
      }
    },
    // Update existing node in chart data
    updateNode: function (targetId, dataObj, arrNodes = null) {
      if (!arrNodes) {
        // arrNodes = this.options.data.children;
        arrNodes = [this.options.data];
      }
        ...
        if (node.id == targetId) {
          Object.keys(dataObj).forEach(key => {
            if (Object.hasOwn(node, key)) {
              node[key] = dataObj[key];
            }
          });
          // console.log('node:', node)
          return node;
        } else if (node.children) {
          const result = this.updateNode(targetId, dataObj, node.children);
          if (result) return result;
        }
      }
    },
    // Delete node from chart data
    deleteNode: function (targetId, arrNodes = null) {
      if (!arrNodes) {
        // arrNodes = this.options.data.children;
        arrNodes = [this.options.data];
      }
      for (const [index, node] of arrNodes.entries()) {
        if (node.id == targetId) {
          arrNodes.splice(index, 1)
          return true;
        } else if (node.children) {
          const result = this.deleteNode(targetId, node.children);
          if (result) break;
        }
      }
    },
